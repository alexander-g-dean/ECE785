...
static int flash(void *arg){
   printk(KERN_INFO "ERPi LED2: Thread has started running \n");
   while(!kthread_should_stop()){           // Returns true when kthread_stop() is called
      set_current_state(TASK_RUNNING);
      if (mode==FLASH) ledOn = !ledOn;      // Invert the LED state
      else if (mode==ON) ledOn = true;
      else ledOn = false;
      gpiod_set_value(descLED, ledOn);       // Use the LED state to light/turn off the LED
      set_current_state(TASK_INTERRUPTIBLE);
      msleep(blinkPeriod/2);                // millisecond sleep for half of the period
   }
   printk(KERN_INFO "ERPi LED2: Thread has run to completion \n");
   return 0;
}
...
static int __init erpi_LED_init(void){
   ...
   printk(KERN_INFO "ERPi LED2: Initializing the ERPi LED LKM\n");
   ...
   if(!erpi_kobj){
      printk(KERN_ALERT "ERPi LED2: failed to create kobject\n");
      return -ENOMEM;
   }
   // add the attributes to /sys/erpi/ -- for example, /sys/rpi/led17/ledOn
   status = sysfs_create_group(erpi_kobj, &attr_group);
   if(status) {
      printk(KERN_ALERT "ERPi LED2: failed to create sysfs group\n");
...
*/
   ...
   if(IS_ERR(task)){                                     // Kthread name is LED_flash_thread
      printk(KERN_ALERT "ERPi LED2: failed to create the task\n");
      return PTR_ERR(task);
   }
   return status;
}
...
static void __exit erpi_LED_exit(void){
   ...
   kobject_put(erpi_kobj);                  // clean up -- remove the kobject sysfs entry
   gpiod_set_value(descLED, 0);             // Turn the LED off, indicates device was unloaded
   gpiod_put(descLED);                      // Free the LED GPIO
   printk(KERN_INFO "ERPi LED2: Goodbye from the ERPi LED LKM!\n");
}
