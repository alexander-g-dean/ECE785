...
extern YUV_T black, white, green, orange, blue, light_brown, pink, dark_pink, yellow;

typedef struct {
  int w, s, h, half_w, half_h, half_s;
  uint8_t *bY;
  uint8_t *bU;
  uint8_t *bV;
} YUV_IMAGE_T;
...
void YUV_Rotate_Image(YUV_IMAGE_T * dst, YUV_IMAGE_T * src, int cX, int cY, float dTheta);
int YUV_Rotate_Translate_Image(YUV_IMAGE_T * dst, YUV_IMAGE_T * src, int cX, int cY,
				float radTheta);
#endif // YUV_H
