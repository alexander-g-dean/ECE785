...
YUV_T black={0, 128, 128}, white={255, 128, 128}, green={128, 135, 64},
  blue={80, 160, 80}, orange={128, 101, 192}, light_brown={128, 90, 158},
  pink={180, 200, 200}, dark_pink={30, 200, 200}, yellow={225,0,148};

void YUV_Image_Init(YUV_IMAGE_T * i, unsigned char * buffer, int width, int stride, int height) {
  // Assumes U, V are 1/4 size of Y
  i->bY = (uint8_t *) buffer;
  i->bU = &(i->bY[stride*height]);
  i->bV = &(i->bU[stride*height/4]);
  i->w = width;
  i->half_w = width/2;
  i->s = stride;
  i->half_s = stride/2;
...
void YUV_Image_Info(YUV_IMAGE_T * i) {
  printf("YUV_IMAGE_T at 0x%lx:\n\tw=%d \th=%d \tbY=0x%lx \tbU=0x%lx \tbV=0x%lx\n",
	 (unsigned long int) i, i->w, i->h, (unsigned long int) i->bY,
	 (unsigned long int) i->bU, (unsigned long int) i->bV);
}

void Set_Pixel(YUV_IMAGE_T * i, int px, int py, uint8_t lum, uint8_t u, uint8_t v){
  int half_px = px/2, half_py = py/2;
  i->bY[px + py*i->s] = lum;
  i->bU[half_px+half_py*i->half_s] = u;
  i->bV[half_px+half_py*i->half_s] = v;
}

void Set_Pixel_yuv(YUV_IMAGE_T * i, int px, int py, YUV_T * yuv) {
  int half_px = px/2, half_py = py/2;
  i->bY[px + py*i->s] = yuv->y;
  i->bU[half_px+half_py*i->half_s] = yuv->u;
  i->bV[half_px+half_py*i->half_s] = yuv->v;
}

void Get_Pixel_yuv(YUV_IMAGE_T * i, int px, int py, YUV_T * yuv) {
  int half_px = px/2, half_py = py/2;
  yuv->y = i->bY[px + py*i->s];
  yuv->u = i->bU[half_px+half_py*i->half_s];
  yuv->v = i->bV[half_px+half_py*i->half_s];
}
...
void YUV_Image_Copy(YUV_IMAGE_T * dst, YUV_IMAGE_T * src) {
  ...
  */
  ...
  memcpy(dst->bY, src->bY, 1.5*(src->s * src->h));
}
...
void YUV_Image_Fill(YUV_IMAGE_T * dst, YUV_T * c) {
  ...
  */
  ...
  memset(dst->bY, c->y, dst->s * dst->h);
  memset(dst->bU, c->u, dst->half_s * dst->half_h);
  memset(dst->bV, c->v, dst->half_s * dst->half_h);
}

int YUV_Rotate_Translate_Image(YUV_IMAGE_T * dst, YUV_IMAGE_T * src,
			       int cX, int cY, float radTheta){
  // Assumes source and destination are same size
  // (cX, cY) Center of rotation
  // Translates center of rotation to center of image
  // Returns number of pixels skipped since outside of source image
  int srcX, srcY, dstX, dstY, tX, tY;
  int h = src->h, w = src->w;
  float sina = sinf(radTheta);
  float cosa = cosf(radTheta);
  int difX, difY;
  YUV_T pixcolor;
  int pix_skipped=0;
  ...
  if (show_data > 0) {
    printf("YUV_Rotate_Translate_Image about (%d,%d) by %f, ", cX, cY, radTheta*180/M_PI);
    printf("offset (%d, %d)\n", tX, tY);
  }
  ...
  for (dstY = 0; dstY < h; dstY++) {
    ...
    for (dstX = 0; dstX < w; dstX ++) {
      difX = cX - dstX - tX;
      srcX = cX + (int32_t)(-difX*cosa - difY*sina);
      srcY = cY + (int32_t)(-difY*cosa + difX*sina);
      if (srcX < 0 || srcX >= w || srcY < 0 || srcY >= h) {
	pixcolor = black;
	pix_skipped++;
      }
      else
	Get_Pixel_yuv(src, srcX, srcY, &pixcolor); // doesn't range check x, y 
      Set_Pixel_yuv(dst, dstX, dstY, &pixcolor);
    } // for dstX
  } // for dstY
  return pix_skipped;
}
...
#if 0 // Disable function -- not ready yet, still buggy
